...
Makes top-level calls use background execution when the request does not explicitly set `async`. Enabled by default; set `asyncByDefault: false` to restore foreground-by-default behavior. Callers can still force foreground with `async: false` unless `forceTopLevelAsync` is enabled.
